import { Vial, ScheduleRow, SimulationResult, BacWaterItem, SyringeConfig, ProtocolStep } from '../types';
...
export const generateSchedule = (
  ...
  const syringeConfig = { ...syringeConfigInput, costPerBox: syringeConfigInput.costPerBox * discountFactor };
  
  // Inventory State
  let peptideStock = vials.map(v => ({ ...v, currentMg: v.mg }));
  let currentVialIndex = 0;
  
  // Bac Water State
  let currentBawIndex = 0;
  const safeBawInventory = bawInventory.length > 0 ? bawInventory : [{id: 'default', name: 'Default Water', sizeMl: 10, cost: 0}];
  let bawStockMl = 0; 
  ...
  const getDoseForWeek = (pWeek: number): number => {
    ...
    const step = protocolSteps.find(s => 
      pWeek >= s.startWeek && (s.endWeek === null || pWeek <= s.endWeek)
    );
  ...
  let running = true;
  let totalWeeksToSimulate = 104; // 2 years cap
  ...
  while (running && week <= totalWeeksToSimulate) {
    ...
    // --- 2. Vial & Water Check ---
    let vialMgRemainingBefore = currentVial.currentMg;
